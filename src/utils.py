...
def display_integration_question(question_num, question_text, key, services, headers):
    ...
    if selected == 'SI':
        ...
        st.write(
            ...
            <div class="grid-container-{key}">
              {''.join([f'<div class="grid-item-{key} grid-header-{key}">{header}</div>' for header in headers])}
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        for i, service in enumerate(services):
            st.write(
                ...
                <div class="grid-container-{key}">
                  <div class="grid-item-{key} service-name-{key}">{service}</div>
                  {''.join([f'<div class="grid-item-{key}"><input type="checkbox" name="service_{key}_{i}_option_{j}"></div>' for j in range(len(headers) - 1)])}
                </div>
                """,
                unsafe_allow_html=True
            )
...
def display_payment_reconciliation_question(question_num, question_text, key, services):
    st.subheader(f'{question_num} {question_text}')

    # Create grid for the question
    st.write(
        f"""
        <style>
        .grid-container-{key} {{
        ...
        <div class="grid-container-{key}">
        <div class="grid-item-{key} grid-header-{key}">Servizi che richiedono<br>pagamenti</div>
        <div class="grid-item-{key} grid-header-{key}">Riconciliazione manuale</div>
        <div class="grid-item-{key} grid-header-{key}">Riconciliazione automatica</div>
        </div>
        """,
        unsafe_allow_html=True
    )
